...
[params,ct_area,bg_area, fg_area, area_resize_factor] = initializeAllAreas(im, params);

% in runTracker we do not output anything because it is just for debug
params.fout = -1;
% set the hashing filter size
params.filterSize=25;
% start the actual tracking
results = tracker(params, im,ct_area, bg_area, fg_area, area_resize_factor);
fclose('all');
end